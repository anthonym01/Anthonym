...
let player = {//Playback control
    ...
    initalize: async function () {
        ...
        song_progress_bar.addEventListener('mouseenter', function (e) {
            clearInterval(player.seekterval)
        })
        song_progress_bar.addEventListener('mouseleave', function (e) {
            player.seekterval = setInterval(() => {
                var seek = player.stream1.seek()
                song_progress_bar.value = seek;
                console.log('got seek ', seek)
            }, 1000)
        })
        ...
        */
        ...
        player.stream1 = new Howl({
            ...
            onplay: async function () {
                ...
                player.seekterval = setInterval(() => {
                    song_progress_bar.value = player.stream1.seek();
                }, 1000)
    ...
    previous: function () {
        console.log('Play Previous',player.stream1.seek());
        
        if(player.playstate == true){
            if(player.stream1.seek()>5){//reset seek on back if less 
